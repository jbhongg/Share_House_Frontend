	...
	<div class="col-lg-6" align="center">
		<div id="userform">
			<div class="form-group" align="left">
				...
				<input type="text" class="form-control" id="name" name="name" v-model="member.name" ref="member.name">
			</div>
			<div class="form-group" align="left">
				<label for="">아이디</label>
			...
			<div class="form-group" align="left">
				...
				<input type="password" class="form-control" id="password" name="password" v-model="member.password" ref="member.password">
			</div>
			<div class="form-group" align="left">
				<label for="">비밀번호재입력</label>
				<input type="password" class="form-control" id="pwdcheck" name="pwdcheck" v-model="pwdcheck" ref="pwdcheck">
			</div>
			<div class="form-group" align="left">
			...
			<div class="form-group" align="left">
				...
				<div id="addressdiv" class="custom-control-inline">
					<button type="button" id="zipcode" name="zipcode" style="background-color: rgb(228, 222, 222);" class="btn" @click="searchaddr">주소 찾기</button>
				</div>
			...
			<div class="form-group" align="center">
				<button type="button" class="btn btn-primary" id="registerBtn" @click="checkValue">회원가입</button>
...
export default {
    ...
    methods: {
        ...
        checkValue() {
      		let err = true;
      		let msg = "";
      		!this.member.name && ((msg = '이름 입력해주세요'), (err = false), this.$refs.member.name.focus());
      			err &&
        	...
        	!this.member.password && ((msg = '비빌번호 입력해주세요'), (err = false), this.$refs.member.password.focus());
      			err &&
        	!this.pwdcheck && ((msg = '비빌번호를 다시 한번 입력해주세요'), (err = false), this.$refs.pwdcheck.focus());
      			err &&
			this.pwdcheck !== this.member.password && ((msg = '비빌번호가 일치하지 않습니다'), (err = false), this.$refs.pwdcheck.focus());
        		err &&
			!this.emailid && ((msg = '이메일 입력해주세요'), (err = false), this.$refs.emailid.focus());
      			err &&
        	(!this.tel1 || !this.tel2 || !this.tel3) && ((msg = '전화번호 입력해주세요'), (err = false), this.$refs.tel1.focus());
      			err &&
        	!this.address_detail && ((msg = '주소 입력해주세요'), (err = false), this.$refs.address_detail.focus());
			this.member.tel = this.tel1 + "-" + this.tel2 + "-" + this.tel3;
